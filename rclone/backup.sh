...
BACKUP_REMOTE_NAME="${WEBDAV_REMOTE_NAME}_backup"
...
# EPUB: 今まで通り。親がなければ mywebdav:epub
...
# ZIP: 親があればそれを使う。なければ bisync と同じ WEBDAV_PATH の下に zip/
if [ -n "$_zip_parent" ]; then
  ZIP_REMOTE_FULL="${WEBDAV_REMOTE_NAME}:${_zip_parent}/zip"
else
  ZIP_REMOTE_FULL="${WEBDAV_REMOTE_NAME}:${WEBDAV_PATH}/zip"
fi
...
# バックアップは「別名リモート」の同じパス配下に置く
BACKUP_ROOT_REMOTE="${BACKUP_REMOTE_NAME}:${WEBDAV_PATH}/_archive/data"
...
# --------------------------------------------------
# rclone.conf 作成（メインとバックアップで2セクション定義）
# --------------------------------------------------
setup_rclone_conf() {
